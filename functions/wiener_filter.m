function [filtered_trace, event_times, event_sizes] = wiener_filter(trace, params, template, nfft)
%
...
if params.event_sign == -1
    for j = 1:length(event_times)
        baseline = mean(trace(max(1,event_times(j)-40):event_times(j))); %changed this to make amplitude more realistic
    ...
    for k = 1:length(event_times)
        baseline = mean(trace(max(1,event_times(k)-40):event_times(k)));
