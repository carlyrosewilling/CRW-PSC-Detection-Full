function [filtered_trace, event_times, event_sizes] = wiener_filter_stim(trace, params, template, nfft)
%
...
if params.event_sign == -1
    for j = 1:length(event_times)
        baseline = mean(trace(max(1,event_times(j)-40):max(event_times(j),length(trace)))); %changed this to make amplitude more realistic
        event_sizes(j) = (min(trace(event_times(j):min(event_times(j)+ min_window,length(trace)))) - baseline) * params.event_sign; %edited this to make the amplitude more realistic
        if min(trace(event_times(j):min(event_times(j)+min_window, length(trace)))) > baseline
            event_sizes(j) = 0;
        end
    end
else
    for k = 1:length(event_times)
        baseline = mean(trace(max(1,event_times(k)-40):max(event_times(k), length(trace))));
        event_sizes(k) = (max(trace(event_times(k):min(event_times(k)+min_window, length(trace)))) - baseline);
        if max(trace(event_times(k):min(event_times(k)+min_window, length(trace)))) < baseline
            event_sizes(k) = 0;
        end
    end
end

event_times(event_sizes < 10) = [];
event_sizes(event_sizes < 10) = [];
