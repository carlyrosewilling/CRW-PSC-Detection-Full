function [filtered_trace, event_times, event_sizes] = wiener_filter_biphasic(trace, params, template, nfft)
%
...
if params.event_sign == -1
    for j = 1:length(event_times)
        baseline = mean(trace(max(1,event_times(j)-20):event_times(j))); %changed this to make amplitude more realistic
        ...
        if min(trace(event_times(j):min(event_times(j)+min_window, length(trace)))) > (.5*baseline)
            event_sizes(j) = 0;
        end
    end
else
    for k = 1:length(event_times)
        baseline = mean(trace(max(1,event_times(k)-20):event_times(k)));
        event_sizes(k) = (max(trace(event_times(k):min(event_times(k)+min_window, length(trace)))) - baseline);
        if max(trace(event_times(k):min(event_times(k)+min_window, length(trace)))) < (.5*baseline)
            event_sizes(k) = 0;
        end
    end
end
