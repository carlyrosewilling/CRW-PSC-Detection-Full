function varargout = AR_PPR(varargin)
% AR_PPR MATLAB code for AR_PPR.fig
%      AR_PPR, by itself, creates a new AR_PPR or raises the existing
%      singleton*.
%
%      H = AR_PPR returns the handle to a new AR_PPR or the handle to
%      the existing singleton*.
%
%      AR_PPR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in AR_PPR.M with the given input arguments.
%
%      AR_PPR('Property','Value',...) creates a new AR_PPR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before AR_PPR_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to AR_PPR_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help AR_PPR
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @AR_PPR_OpeningFcn, ...
                   'gui_OutputFcn',  @AR_PPR_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before AR_PPR is made visible.
function AR_PPR_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to AR_PPR (see VARARGIN)

% Choose default command line output for AR_PPR
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes AR_PPR wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = AR_PPR_OutputFcn(hObject, eventdata, handles) 
...
axes(handles.axes2);
cla;
plot(I.peristim2, 'Color', [0.3010, 0.7450, 0.9330]);
hold on;
scatter(I.event_times2, repmat(mean(I.peristim2),1,length(I.event_times2)), 50, [1, 0.33, 0.64]);
title(strcat('Peristimulus 2 ', I.params.traces_file(1:end-4), ' Data with Events'), 'Interpreter', 'none');
xlabel('Frames (1/10000 s)');
ylabel('mV');
ylim auto
xlim([0 length(I.peristim2)]);
